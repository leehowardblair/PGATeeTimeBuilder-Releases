...
# PGA Tee Time Builder v1.2.1

## 📝 Important installation note
- Because the faulty updater handoff any older installed copies, **v1.1.5 and v1.2.0 users must install v1.2.1 manually once**. Future releases will use the corrected one-click updates.

## ✨ Highlights
- Fully signed and notarized Universal macOS release for Apple Silicon and Intel Macs.
- Intel macOS Monterey startup compatibility is now included in the main macOS app.
...
## 🍎 Intel macOS Monterey startup reliability
- On affected Intel macOS 12 Monterey machines, the app now prepares a private libffi execution directory before optional native Python bridge imports.
...
## 🐞 Updater Hotfix
- Fixed a Windows and macOS one-click updater handoff mismatch that could download a release package successfully but then stop before launching the installer or app-replacement helper.
- Windows now hands off the staged installer after the current app exits.
- macOS now hands off the signed Universal app ZIP after the current app exits.
- The update workspace continues to live under the app’s persistent per-user data folder, never inside a PyInstaller temporary folder.
...
**Windows**  
`PGA_TeeTime_Builder_Setup_v1.2.1.exe`

**macOS**  
`PGA_TeeTime_Builder_macOS_v1.2.1.zip`
